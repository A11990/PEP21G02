...
- Iterati obiectul creat si salvati fiecare valoare din iteratie in acelasi fisier pe linie noua. 30p
3) Documentatie: 20p
...
class CarFactory:
    """doc for class"""
    def _init_(self, series: int, num_pieces: int):
        """doc for method"""
        self.series = series
        self.num_pieces = num_pieces

    def series_left_hand(self):
        """doc for method"""
        list_left_hand = []
        for i in range(self.series, (self.series + self.num_pieces) + 1):
            if i % 2 == 1:
                list_left_hand.append(i)
        return list_left_hand


    def series_right_hand(self):
        """doc for method"""
        list_right_hand = []
        for i in range(self.series, (self.series + self.num_pieces) + 1):
            if i % 2 == 0:
                list_right_hand.append(i)
        return list_right_hand
    ...
    def _iter_(self):
        ...
        for i in range(self.series, (self.series + self.num_pieces) + 1):
            result.add(i // 20)
        return CarFactoryIter(result)

class CarFactoryIter:
    """doc for class"""
    def _init_(self, data: list):
        """doc for method"""
        self.data = data
...
car = CarFactory(314, 90)

with open('homework6.txt', 'w') as file:
    for x in car:
        file.write(str(x) + '\n')